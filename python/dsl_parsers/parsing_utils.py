...
def getNameNumber(aalist_):
    aalist = [interface[0] for interface in aalist_]
    comm = ['ice' if len (interface) == 1 else interface[1] for interface in aalist_]

    temp = [x+'#'+y for (x, y) in zip (aalist, comm)]
