...
for name, num in getNameNumber(component['requires']):
	if communicationIsIce(name):
		...
		w = REQUIRE_STR.replace("<C++_VERSION>", cpp).replace("<NORMAL>", name[0]).replace("<LOWER>", name[0].lower()).replace("<PROXYNAME>", name[0].lower()+num).replace("<PROXYNUMBER>", num)
		cog.outl(w)
		if component['language'].lower() == "cpp":
			cog.outl("<TABHERE>mprx[\""+name[0]+"Proxy"+num+"\"] = (::IceProxy::Ice::Object*)(&"+name[0].lower()+num+"_proxy);//Remote server proxy creation example");
		else:
			proxy_list.append(name[0].lower()+num+"_proxy")
